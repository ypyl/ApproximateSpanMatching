...
When an `IAlignmentStrategy` uses a word similarity function and produces matched pairs, each `MatchedPair` SHALL include the similarity score between the matched query and document words.

```csharp
public readonly record struct MatchedPair(int QueryIndex, int DocIndex, double Similarity);
```

- For exact matches where `wordSimilarity` is `null`, `Similarity` SHALL be 1.0.
- For exact matches where a similarity function is provided and `Similarity(w1, w2) == 1.0`, `Similarity` SHALL be 1.0.
- For fuzzy matches, `Similarity` SHALL be the computed similarity ∈ (0.0, 1.0).

#### Scenario: Exact match pair carries similarity 1.0

- **WHEN** an alignment produces a matched pair for identical words "brown" ↔ "brown"
- **THEN** the `MatchedPair.Similarity` is 1.0

#### Scenario: Fuzzy match pair carries computed similarity

- **WHEN** an alignment produces a matched pair for "qu1ck" ↔ "quick" using trigram Jaccard
- **AND** the computed similarity is approximately 0.25
- **THEN** the `MatchedPair.Similarity` is approximately 0.25

#### Scenario: Null word similarity defaults pairs to 1.0
